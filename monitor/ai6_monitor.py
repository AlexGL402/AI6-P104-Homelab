...
def run_workerctl(*args):
    p = subprocess.run(
        ["sudo", "/usr/local/sbin/ai6-workerctl", *args],
...
def _service_for_port(port: int):
    for name in (f"llama2-{port}", f"llama-{port}"):
        if systemd_active(name):
            return name
    return None
...
def worker_details(port: int):
    ...
    info = {
        "active": active, "ready": state == "ready", "state": state,
        "status_text": status_text, "model": None, "last_tok_s": None,
        "last_prompt_tok_s": None, "last_prompt_tokens": None, "uptime_s": None,
    }
    if not active:
        return info
...
def collect_stats():
    ...
    workers = {str(p): worker_details(p) for p in (8081, 8082, 8083, 8092)}
    ...
    return {
        ...
        "disk": {"used_bytes": root.used, "total_bytes": root.total, "free_bytes": root.free, "usage_pct": root.percent},
        "network": {"bytes_sent": net.bytes_sent, "bytes_recv": net.bytes_recv},
        "gpu": {
            "count": len(gpus), "power_total_w": round(sum(powers), 2),
        ...
        "llama": {
            ...
            "8081": workers["8081"]["active"], "8082": workers["8082"]["active"],
            "8083": workers["8083"]["active"], "8092": workers["8092"]["active"],
            "workers": workers,
        },
    }
...
def psu_sample(sample: PsuSample):
    ...
    row = {
        "timestamp": stats["timestamp"], "voltage_12v": sample.voltage_12v,
        "gpu_power_total_w": stats["gpu"]["power_total_w"],
        "gpu_temp_max_c": stats["gpu"]["temperature_max_c"],
        "cpu_usage_pct": stats["cpu"]["usage_pct"], "ram_usage_pct": stats["memory"]["usage_pct"],
        "note": sample.note,
    }
...
async function refresh(){
 ...
 workers.innerHTML=ports.map(p=>stateIcon(ws[String(p)]||{})).join(' ');profile.textContent='profile '+(s.llama.profile==='222'?'2+2+2':'3+3')+' • ✅ Ready  ⏳ Loading  ❌ Down';
 ...
 const tw=ws['8092']||{};const tmodel=tw.model||((tw.state==='loading')?'loading model…':'model ?');testworker.innerHTML='<div class="testrow"><b>TEST 8092</b> <span class="status '+(tw.state||'down')+'">'+stateIcon(tw)+' '+(tw.status_text||'Down')+'</span> <span class="muted">• manual benchmark server</span><br><span class="muted">'+tmodel+(tw.last_tok_s!=null?' • '+tw.last_tok_s.toFixed(2)+' tok/s':'')+'</span></div>';
